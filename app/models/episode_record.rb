...
class EpisodeRecord < ApplicationRecord
  ...
  counter_culture :episode, column_name: ->(episode_record) {
                                           episode_record.body.present? ? :episode_record_bodies_count : nil
                                         }
  ...
  def share_url
    "#{user.preferred_anikuto_url}/@#{user.username}/records/#{record.id}"
  end
