...
module ControllerCommon
  ...
  included do
    ...
    if ENV.fetch('ANIKUTO_BASIC_AUTH') == 'on'
      name = ENV.fetch('ANIKUTO_BASIC_AUTH_NAME')
      password = ENV.fetch('ANIKUTO_BASIC_AUTH_PASSWORD')
    ...
    def switch_locale(&action)
      ...
      when ENV.fetch('ANIKUTO_JP_DOMAIN')
    ...
    def discord_invite_url
      ENV.fetch('ANIKUTO_DISCORD_INVITE_URL')
    end
  end
end
