...
describe Beta::Types::Objects::UserType do
  describe "programs" do
    ...
    let(:result_program_ids) do
      result = Beta::AnikutoSchema.execute(query_string)

      pp(result) if result["errors"]

      result.dig("data", "user", "programs", "nodes").map { |node| node["anikutoId"] }
    end
      ...
      context "when the user does not set status on works" do
        context "when the `unwatched` option is not specified" do
          ...
          it "returns no slots" do
            expect(result_program_ids).to eq([])
          end
        end
        ...
        context "when the user is watching work_2" do
          ...
          context "when the user does not watch episodes" do
            context "when the `unwatched` option is not specified" do
              ...
              it "returns slots" do
                expect(result_program_ids).to contain_exactly(slot_3.id, slot_4.id, slot_7.id, slot_8.id)
              end
            end
          ...
          context "when the user watches episode_1" do
            ...
            context "when the `unwatched` option is `false`" do
              ...
              it "returns slots" do
                expect(result_program_ids).to contain_exactly(slot_7.id)
              end
            end
            ...
            context "when the `unwatched` option is `true`" do
              ...
              it "returns slots" do
                expect(result_program_ids).to contain_exactly(slot_3.id, slot_4.id, slot_8.id)
              end
            end
          end
        end
